      ...
      async function savePlayerToFile(playerData) {
        try {
          ...
          if (!saved) {
            console.warn(
              "Không thể lưu vào OPFS hoặc FS; tiếp tục với localStorage"
            );
            return false;
          }
        ...
        try {
          ...
          if (
            ...
            try {
              ...
              console.warn(
                "Không thể xóa file (có thể đã bị xóa trước đó):",
                err
              );
            }
          }
      ...
      async function savePlayerSession(nickname, avatarIndex = 1) {
        // Generate player data - 3 trường: id, name, avatar
        const playerId = generatePlayerId();
        const DEFAULT_AVATAR = "../../assets/images/daden.jpg";
        // Use default avatar since avatar selection UI is not present
        const selectedAvatar = DEFAULT_AVATAR;
        const playerData = {
          id: playerId,
          name: nickname,
          // Store absolute path to ensure it works across pages (with default fallback)
          avatar: toAbsolutePath(selectedAvatar),
        };

        // Save to localStorage (giữ nguyên để session hoạt động)
        localStorage.setItem(
          "battleship-player-session",
          JSON.stringify(playerData)
        );

        // Save player data to JSON file and wait for completion
      ...
      window.addEventListener("load", function () {
        ...
        // also ensure no stale file markers remain
        sessionStorage.removeItem("bs-keep-file");
      });

      // Clear all data when page is about to unload (F5, close, navigate away)
      window.addEventListener("beforeunload", async function (e) {
        const keep = sessionStorage.getItem("bs-keep-file") === "1";
        console.log("Page unloading - keep file?", keep);
        if (!keep) {
      ...
      // Alternative for when page is hidden/closed
      window.addEventListener("pagehide", async function (e) {
        const keep = sessionStorage.getItem("bs-keep-file") === "1";
        console.log("Page hidden - keep file?", keep);
        if (!keep) {
          await cleanupSavedFiles();
          localStorage.removeItem("battleship-player-session");
        }
      ...
      // Clear on browser tab close/refresh
      window.addEventListener("unload", async function (e) {
        const keep = sessionStorage.getItem("bs-keep-file") === "1";
        console.log("Page unloaded - keep file?", keep);
        if (!keep) {
          await cleanupSavedFiles();
          localStorage.removeItem("battleship-player-session");
        }
      ...
      // Avatar functionality - removed since avatar UI elements don't exist in the current HTML
      // But we'll keep the currentAvatarIndex for session saving

      // Note: No need to check for existing session since localStorage is cleared on every page load
