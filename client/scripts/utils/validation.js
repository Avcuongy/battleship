...
const Validation = {
    ...
    for (const ship of fleet) {
      ...
      for (const pos of positions) {
        if (
          ...
          errors.push(
            `${ship.shipType} out of bounds at (${pos.posRow}, ${pos.posCol})`
          );
        }
      }
    }
  ...
  checkOrientations(fleet) {
    ...
    for (const ship of fleet) {
      if (!validOrientations.includes(ship.shipOrientation)) {
        errors.push(
          `${ship.shipType} has invalid orientation: ${ship.shipOrientation}`
        );
      }
    }
  ...
  isValidPosition(position) {
    return (
      position.posRow >= 0 &&
      position.posRow < this.BOARD_SIZE &&
      position.posCol >= 0 &&
      position.posCol < this.BOARD_SIZE
    );
  },
};
